...
$app->put('/register/{username}', function ($request, $response) {
	$input = $request->getParsedBody();
...
$app->put('/register_shop/{idusers}', function ($request, $response) {
	$input = $request->getParsedBody();
	$sql = "UPDATE users SET shop_email = :shop_email, shop_name = :shop_name,
			shop_zip = : shop_zip, description = :description WHERE idusers = :idusers AND shop_name = :shop_name";
	$sth = $this->db->prepare($sql);
    $sth->bindParam("shop_email", $input['shop_email']);
    $sth->bindParam("shop_name", $input['shop_name']);
    $sth->bindParam("shop_zip", $input['shop_zip']);
...
$app->post('/add_vehicle/{idusers}', function ($request, $response) {
    $input = $request->getParsedBody();
    $sql = "UPDATE vehicle
        SET idusers = :idusers, make=:make, model=:model, year=:year, miles=:miles, oil_change_miles=:oil_change_miles,
        tire_change_miles=:tire_change_miles, transmission_check_miles=:transmission_check_miles,
        last_inspection_date=:last_inspection_date, general_description=:general_description WHERE
        idusers = :idusers AND miles = :miles";
    $sth = $this->db->prepare($sql);
    $sth->bindParam("idusers", $input['idusers']);
    $sth->bindParam("make", $input['make']);
    $sth->bindParam("model", $input['model']);
